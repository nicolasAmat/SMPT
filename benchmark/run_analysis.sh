...
while IFS= read instance; do
    ...
    # Run smpt
    echo "smpt --show-techniques --show-time --timeout $TIMEOUT ${PATH_INSTANCE}model.net --xml ${PATH_INSTANCE}ReachabilityCardinality.xml > ${PATH_OUTPUT}RC_without_reduction.out" >> $TEMP_FILE
    echo "smpt --show-techniques --show-time --timeout $TIMEOUT ${PATH_INSTANCE}model.net --xml ${PATH_INSTANCE}ReachabilityFireability.xml > ${PATH_OUTPUT}RF_without_reduction.out" >> $TEMP_FILE
    echo "smpt --show-techniques --show-time --timeout $TIMEOUT ${PATH_INSTANCE}model.net --deadlock > ${PATH_OUTPUT}RD_without_reduction.out" >> $TEMP_FILE
